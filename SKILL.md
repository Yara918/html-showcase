---
name: html-showcase
description: Create stunning, animation-rich single-file HTML pages / dashboards / presentations from scratch. Use when the user wants a beautiful HTML page, a data dashboard, a slide deck, or a presentation (做页面、做看板、做演示、做 slides、HTML 页面、网页). Helps non-designers discover their aesthetic through visual exploration rather than abstract choices.
---
...
**Preview gate (NON-NEGOTIABLE — applies to every model and every platform):**

- You MUST render the 3 style previews and collect the user's choice BEFORE generating any part of the full deck. Skipping the preview step is a hard violation: do not jump straight to the final deck, do not "save time", do not generate the full deck in the background while asking.
- The preview step is not optional even when the user seems impatient or says "just make it". If the user explicitly refuses to look at previews, still render the 3 previews, pick the best-matched style yourself, state your choice in plain words ("我按默认用 Style B — XXX，你想换随时说"), and only then generate — never generate from an unstated guess.
- If your environment cannot auto-open preview files (no browser / `open` command), render them as files anyway, give the user the file paths, and describe each preview in 2–3 lines (palette, type, layout, signature move) so the choice is still visual.
- Only after the user picks Style A / Style B / Style C / Mix elements (Step 2.1) do you read the chosen template's full `design.md` (or reuse the custom preview's system) and generate the full deck.
...
Save previews to `.html-showcase/slide-previews/` (style-a.html, style-b.html, style-c.html). Each should be self-contained and compact, showing one animated title slide.
...
The user's choice is REQUIRED before full-deck generation (Preview gate above). If the user replies with anything other than a pick — "you choose", "随便", "just do it" — do NOT silently pick behind their back: re-present the three previews once; if they still decline to choose, state the best-matched default explicitly ("我按默认用 Style B — XXX，想换随时说") and proceed only with that stated default.

---

## Phase 3: Generate Presentation

**Guard (NON-NEGOTIABLE):** Do not enter Phase 3 until the user has chosen a style in Step 2.1 (or explicitly accepted a stated default). If no choice exists yet, return to Step 2.1 — do not generate the full deck without a chosen style.
...
- Keep the output as a single self-contained HTML file.
...
1. **Clean up** — Delete `.html-showcase/slide-previews/` if it exists
