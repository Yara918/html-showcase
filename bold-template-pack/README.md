...
This pack brings 20 curated HTML design systems into the `html-showcase` skill.
Every template follows the same contract: a lightweight `preview.md` for style
selection and a full `design.md` system recipe used only after the user picks
that template.

Each system is a self-contained design language: a committed palette, a
deliberate type pairing, a recognizable layout system, and at least one
signature move. The styles draw on public design-history references — Swiss
International Typography, Bauhaus geometry, dense financial-data annotation,
glassmorphism, split-flap departure boards, CRT terminals, museum gallery
labels, Risograph printing, Japanese minimalism, transit-map diagrams, kanban
walls, stadium scoreboards, and engineering drafting — and are re-authored as
original generation specifications for this skill.
...
3. For title-slide previews, read only the relevant candidate `preview.md`
   files.
4. After the user chooses a template, read exactly that one template's full
   `design.md`.
5. Do not read every `design.md` in the pack.
6. Do not read or copy any `template.html` unless a selected `design.md` is
   missing a critical implementation detail.

## How To Use In html-showcase

Preview mix:

- 1 safe option from `STYLE_PRESETS.md`
- at least 1 bold option from this pack
- 1 wildcard option, which may be another pack template or a self-generated
  custom design
...
- For board, legal, regulatory, healthcare, investor-update, or highly formal
  ...
  higher-formality templates. The wildcard should feel authoritative and
  specific, not merely decorative.
- For bold, editorial, expressive, experimental, or highly designed decks, keep
  the safe option as a readable fallback, choose one strong template, and use
  the wildcard for either a second adventurous template or a custom design
  that better matches the user's occasion and vibe.

If the wildcard is custom, it must follow html-showcase's no-slop aesthetics:
...
- Keep html-showcase output as one self-contained HTML file.
- Include the full contents of `viewport-base.css`.
- Generate every deck as a fixed 1920×1080 stage scaled uniformly to the
  viewport. This applies even if the source design was originally
  viewport-fluid.
- Treat `vw`, `vh`, and `clamp()` values in a `design.md` as design
