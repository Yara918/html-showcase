...
- DM Sans for body and JetBrains Mono for every timestamp, kicker, label, and chrome element.
...
- **JetBrains Mono** (`{typography.label.fontFamily}`) — a condensed editorial monospace. Carries every label, every kicker, every chrome element (top bar, footer, slide counter), every chapter number, every mono caption, every stat note, every timeline date. Mono is the system's "metadata voice."
...
| `{typography.label}` | 0.7vw | JetBrains Mono | 500 | Mono uppercase chrome label, kicker, tag, mono metadata |
...
- Use JetBrains Mono for every kicker, label, tag, chrome element, slide counter, chapter number, and statistical note. Mono is the metadata voice.
...
| Label / Kicker / Chrome / Slide counter | JetBrains Mono (500) | **思源黑体 Noto Sans SC** | 500 (uppercase + tracking removed) |
...
- Pangu spacing (盘古之白): `使用 AI` not `使用AI`
